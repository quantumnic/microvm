...
fn execute_v_crypto(cpu: &mut Cpu, raw: u32, inst_len: u64) -> bool {
    ...
    match funct6 {
        ...
        0b101001 if vs1 == 16 => {
            // vsm4r.vs vd, vs2 — SM4 rounds with scalar round key (Zvksed)
            let rk = aes_read_group(cpu, vs2, 0);
            for g in 0..num_groups {
                let base = g * 4;
                let state = aes_read_group(cpu, vd, base);
                let result = sm4_rounds(state, rk);
                aes_write_group(cpu, vd, base, &result);
            }
        }
        0b100001 => {
            // vsm4k.vi vd, vs2, uimm — SM4 key expansion (Zvksed)
            let rnd = (vs1 as u32) & 0x7; // uimm[2:0]
            for g in 0..num_groups {
                let base = g * 4;
                let rk = aes_read_group(cpu, vs2, base);
                let result = sm4_key_expand(rk, rnd);
                aes_write_group(cpu, vd, base, &result);
            }
        }
        0b100010 => {
        ...
        0b101000 if vs1 == 16 => {
            // vsm4r.vv vd, vs2 — SM4 rounds (Zvksed)
            for g in 0..num_groups {
                let base = g * 4;
                let state = aes_read_group(cpu, vd, base);
                let rk = aes_read_group(cpu, vs2, base);
...
/// SM4 SubWord: apply S-box to each byte of a 32-bit word.
#[inline]
fn sm4_subword(x: u32) -> u32 {
    let b0 = SM4_SBOX[(x & 0xFF) as usize] as u32;
    let b1 = SM4_SBOX[((x >> 8) & 0xFF) as usize] as u32;
    let b2 = SM4_SBOX[((x >> 16) & 0xFF) as usize] as u32;
    let b3 = SM4_SBOX[((x >> 24) & 0xFF) as usize] as u32;
    b0 | (b1 << 8) | (b2 << 16) | (b3 << 24)
}

/// SM4 round function: X ^ S ^ ROL32(S,2) ^ ROL32(S,10) ^ ROL32(S,18) ^ ROL32(S,24)
#[inline]
fn sm4_round_fn(x: u32, s: u32) -> u32 {
    x ^ s ^ s.rotate_left(2) ^ s.rotate_left(10) ^ s.rotate_left(18) ^ s.rotate_left(24)
}

/// SM4 key expansion round: X ^ S ^ ROL32(S,13) ^ ROL32(S,23)
#[inline]
fn sm4_key_round_fn(x: u32, s: u32) -> u32 {
    x ^ s ^ s.rotate_left(13) ^ s.rotate_left(23)
}

/// Perform 4 rounds of SM4 encryption/decryption.
fn sm4_rounds(state: [u32; 4], rk: [u32; 4]) -> [u32; 4] {
    let [x0, x1, x2, x3] = state;
    let b = x1 ^ x2 ^ x3 ^ rk[0];
    let s = sm4_subword(b);
    let x4 = sm4_round_fn(x0, s);

    let b = x2 ^ x3 ^ x4 ^ rk[1];
    let s = sm4_subword(b);
    let x5 = sm4_round_fn(x1, s);

    let b = x3 ^ x4 ^ x5 ^ rk[2];
    let s = sm4_subword(b);
    let x6 = sm4_round_fn(x2, s);

    let b = x4 ^ x5 ^ x6 ^ rk[3];
    let s = sm4_subword(b);
    let x7 = sm4_round_fn(x3, s);

    [x4, x5, x6, x7]
}

/// SM4 key expansion: generate 4 round keys from previous 4.
fn sm4_key_expand(rk: [u32; 4], rnd: u32) -> [u32; 4] {
    let base = (rnd * 4) as usize;
    let [rk0, rk1, rk2, rk3] = rk;

    let b = rk1 ^ rk2 ^ rk3 ^ SM4_CK[base];
    let s = sm4_subword(b);
    let rk4 = sm4_key_round_fn(rk0, s);

    let b = rk2 ^ rk3 ^ rk4 ^ SM4_CK[base + 1];
    let s = sm4_subword(b);
    let rk5 = sm4_key_round_fn(rk1, s);

    let b = rk3 ^ rk4 ^ rk5 ^ SM4_CK[base + 2];
    let s = sm4_subword(b);
    let rk6 = sm4_key_round_fn(rk2, s);

    let b = rk4 ^ rk5 ^ rk6 ^ SM4_CK[base + 3];
    let s = sm4_subword(b);
    let rk7 = sm4_key_round_fn(rk3, s);
